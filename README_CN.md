...
也可以直接使用 Task3.2 venv（默认 8 个独立进程并行处理 8 个场景）：
...
脚本会复用家具阶段的 `BlenderRenderer`，在每个 `scene_*/` 目录下生成
`critic_final_views/00_top.png` 和 `critic_final_views/01_side.png`（默认 1536
分辨率、无标签；side 视角按 production renderer 规则隐藏朝向相机的墙）。
输入是完整的 `combined_house/house.blend`，而不是单独家具对象。共享 base 默认跳过；明确处理共享 base 时加
