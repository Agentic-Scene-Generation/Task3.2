...
def main() -> None:
    ...
    for relative, names in [
        (
            "scenesmith/agent_utils/asset_manager.py",
            {
                ...
                "_normalize_axis_string",
                "_hsm_vector_to_blender_front_axis",
        ...
        (
            ...
            {
                "_hssd_annotation_functional_hints",
                "_hssd_asset_id_from_metadata",
    ...
    assert (
        namespace["_normalize_hssd_annotation_front_axis"](
            front["asset_local_front_axis"]
        )
        == "-Y"
    )
    hints = namespace["_hssd_annotation_functional_hints"](
        SimpleNamespace(metadata={"hssd_mesh_id": uid})
    )
    assert hints["asset_annotation_source"] == "3dfuture_annotations", hints[
    ...
    for filename, digest in manifest["sha256"].items():
        assert (
            hashlib.sha256((data / filename).read_bytes()).hexdigest() == digest
        ), filename
    assert (
        hashlib.sha256(
            (data / "canonical_front_required.json.gz").read_bytes()
        ).hexdigest()
    ...
    print(
        json.dumps(
            {
                "passed": True,
                "front_assets_preserved": manifest["total"],
                "sample": uid,
                "asset_local_axis": [0, 0, 1],
                "runtime_axis": "-Y",
