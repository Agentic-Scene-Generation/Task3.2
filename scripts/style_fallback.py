...
def with_style_fallback(annotation):
    ...
    labels=sorted({s['level_1_id'] for s in value.get('styles',[]) if s.get('level_1_id')})
    ...
    if value.get('schema_version')=='asset_style@1.0':
        labels=sorted({s['style_id'] for s in value.get('intrinsic_styles',[]) if s.get('style_id') not in (None,'unspecified','unknown_style')})
    fallback=not labels
    ...
    value['style_fallback']=({'label_id':'unknown_style','label_zh':'风格未识别',
        'reason':'max_judgments_exhausted' if value.get('judgment_count',0)>=value.get('max_judgments',5) else 'inconclusive_or_pending',
        'is_semantic_style':False,'policy':'unknown_style_low_priority@1.0'} if fallback else None)
    return value
...
def rerank_style_candidates(candidates, overlay, top_k):
    ...
    for item in candidates:
        row=dict(item);uid=str(row.get('asset_id') or row.get('asset_uid') or '')
        ...
        if annotation is None:
            ...
            row.update(style_retrieval_priority_tier=0,style_priority_policy_applied=False)
            enriched.append(row)
            continue
        info=with_style_fallback(annotation)
        row.update({k:info[k] for k in ('style_label_ids','style_is_fallback','style_retrieval_priority_tier')})
