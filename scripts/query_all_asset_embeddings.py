...
def main() -> None:
    ...
    parser.add_argument(
        "--model-id", required=True, help="Operator-verified exact backend identity"
    )
    ...
    parser.add_argument(
        ...
        / "scenesmith/scenebenchmark_critic/asset_annotation_data/asset_style_annotations_v2.shared.json.gz",
        help="Non-generated current style labels; unknown_style candidates rank last within the retrieved pool",
    )
    ...
    if isinstance(style_overlay.get("records"), dict):
        style_overlay = style_overlay["records"]
    signature = json.loads(
        args.collection_path.with_name(
            args.collection_path.name + ".input.json"
        ).read_text()
    )
    ...
    if (
        array.shape != (signature["dimension"],)
        or not np.isfinite(array).all()
        or np.linalg.norm(array) == 0
    ):
        raise ValueError("Invalid query vector")
    collection = zvec.open(
        str(args.collection_path), option=zvec.CollectionOption(read_only=True)
    )
    try:
        ...
        print(
            json.dumps(
                rerank_style_candidates(candidates, style_overlay, args.top_k),
                ensure_ascii=False,
                indent=2,
            )
        )
