...
SHARED = Path(
    "/data/task3_2/share_data/scenesmith/all_assets_style_v2_delivery_20260909"
)
...
ROOTS = [
    LOCAL,
    REPO / "data",
    TASK / "scenesmith/scenebenchmark_critic/asset_annotation_data",
]
FILES = [
    "hssd_annotation_lookup.json.gz",
    "3dfuture/3dfuture_annotation_lookup.json.gz",
    "external/others_annotation_lookup.json.gz",
    "generated_annotations.json.gz",
]
...
def resolve(row, candidate=None):
    ...
    status = result.get("front_status", "not_judged")
    previous = horizontal(old.get("canonical_orientation_axis")) or horizontal(
        old.get("asset_local_front_axis")
    )
    axis = previous or [0.0, 0.0, 1.0]
    source = (
        "previous_direction_as_fallback"
        if previous
        else "canonical_positive_z_fallback"
    )
    confidence = 0.2
    semantic = False
    if not c and previous and old.get("canonical_orientation_is_semantic_front"):
        source = "previous_semantic_pending_luna"
        semantic = True
        confidence = old.get(
            "canonical_orientation_confidence", old.get("confidence", 0.2)
        )
    reliable = min(
        result.get("confidence", 0), result.get("up_confidence", 0)
    ) >= 0.7 and not c.get("consistency_flags")
    chosen = None
    if reliable and status == "unique":
        chosen = horizontal(c.get("front_axis_evidence_local"))
    if reliable and status == "multiple_valid":
        chosen = next(
            (
                v
                for a in result.get("alternative_front_axes", [])
                if (v := horizontal(a))
            ),
            None,
        )
    if chosen:
        axis = chosen
        source = (
            "luna_v2_unique" if status == "unique" else "luna_v2_one_of_multiple_valid"
        )
    ...
    return {
        ...
        "canonical_orientation_source": source,
        "canonical_orientation_confidence": confidence,
        "canonical_front_direction": f"{axis} normalized asset-local Y-up",
        "canonical_front_face": (
            "Luna visual semantic face"
            if source.startswith("luna_")
            else "retained or conventional placement face"
        ),
        "up_axis": "y",
        "up_axis_local": [0.0, 1.0, 0.0],
        "confidence": confidence,
        "front_axis_confidence": confidence,
        "method": source,
        "status": (
            "semantic_single_pass" if semantic else "explicit_nonsemantic_fallback"
        ),
        ...
        "luna_front_status": status,
        "luna_consistency_flags": c.get("consistency_flags", []),
        "luna_up_confidence": result.get("up_confidence"),
        "semantic_direction_kind": "semantic_front" if semantic else "fallback_front",
        "semantic_directions": [
            {
                "kind": "semantic_front" if semantic else "fallback_front",
                "axis": axis,
                "axis_frame": "normalized_asset_local_y_up",
                "confidence": confidence,
                "is_primary": True,
                "is_strict_positive_front": False,
            }
...
def uid(key, row, filename):
    if row.get("asset_uid"):
        return row["asset_uid"]
    return (
        ("generated:" + key)
        if filename.startswith("generated")
        else (key if ":" in key else "hssd:" + key)
    )
...
def main():
    ...
    for item in genitems:
        if item["asset_id"] not in generated:
            aid, row = old_export.generated(item, fronts)
            generated[aid] = row
    # Rebase the inherited scale provenance of new generated rows.
    raw = json.dumps(generated, ensure_ascii=False).replace(
        "/data/250010098/abo_production_20260906/scale_reference.json",
        str(LOCAL / "annotation_provenance/scale_reference.json"),
    )
    ...
    for index, root in enumerate(ROOTS):
        for name in FILES:
            p = root / name
            baselines[str(p)] = sha(p)
            dest = backup / str(index) / name
            dest.parent.mkdir(parents=True, exist_ok=True)
            assert not dest.exists(), "backup_exists"
            shutil.copy2(p, dest)
    for name in FILES:
        local = generated if name == FILES[-1] else load(LOCAL / name)
        for key, row in local.items():
            u = uid(key, row, name)
            candidate = fronts.get(u, row.get("front_visual_candidate"))
            if candidate:
                row["front_visual_candidate"] = candidate
            if "canonical_front_previous" not in row:
                row["canonical_front_previous"] = copy.deepcopy(
                    row.get("canonical_front")
                )
            row["canonical_front"] = resolve(row, candidate)
            allfronts[u] = row["canonical_front"]
            counts[u.split(":")[0]] += 1
            methods[row["canonical_front"]["canonical_orientation_source"]] += 1
            if name == FILES[-1]:
                row["missing_families"] = [
                    x for x in row.get("missing_families", []) if x != "canonical_front"
                ]
        ...
        portable = (
            copy.deepcopy(local) if name == FILES[-1] else load(REPO / "data" / name)
        )
        for key, row in portable.items():
            for field in [
                "canonical_front",
                "canonical_front_previous",
                "front_visual_candidate",
            ]:
                if field in local[key]:
                    row[field] = local[key][field]
    ...
    manifest = {
        ...
        "counts": dict(counts),
        "total": len(allfronts),
        "directions": dict(methods),
        "all_assets_have_nonempty_front": True,
        "fallback_is_not_semantic": True,
        "mesh_upright_changed": False,
        "luna_judgments_in_snapshot": len(fronts),
        "generated_index_count": len(genitems),
        "front_quality_certified": False,
        "embedding_vectors_updated": False,
        "scope": "Published inventory at snapshot; new producer outputs require subsequent publication. No Flare Luna requests.",
        "backup": str(backup),
        "documentation": "docs/FRONT_REQUIRED_20260921_ZH.md",
    }
    ...
    for kind in ["shared", "portable"]:
        for name in FILES:
            ...
            for key, row in data.items():
                cf = row["canonical_front"]
                axis = cf["canonical_orientation_axis"]
                assert (
                    horizontal(axis) == axis
                    and abs(sum(x * x for x in axis) - 1) < 1e-6
                )
                assert cf["asset_local_front_axis"] == axis and isinstance(
                    cf["canonical_front_direction"], str
                )
                assert cf == allfronts[uid(key, row, name)]
            assert b"/data/250010098/" not in gzip.decompress(
                (OUT / kind / name).read_bytes()
            ), name
    for index, root in enumerate(ROOTS):
        kind = "shared" if index == 0 else "portable"
        for name in FILES:
            assert (
                sha(root / name) == baselines[str(root / name)]
            ), "consumer_changed_during_export"
            target = root / name
            temp = target.with_name(target.name + ".front-release-tmp")
            shutil.copyfile(OUT / kind / name, temp)
            temp.chmod(0o666)
            temp.replace(target)
        shutil.copyfile(
            OUT / "canonical_front_required.json.gz",
            root / "canonical_front_required.json.gz",
        )
        m = {**manifest, "sha256": manifest[kind + "_sha256"]}
        write(root / MANIFEST, m)
        write(
            root / "CURRENT_ANNOTATION_RELEASE.json",
            {
                "release": RELEASE,
                "manifest": MANIFEST,
                "documentation": manifest["documentation"],
                "front_candidates_are_not_canonical_replacements": False,
                "embedding_vectors_updated": False,
            },
        )
    for p in OUT.rglob("*"):
