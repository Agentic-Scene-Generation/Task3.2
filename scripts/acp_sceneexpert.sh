...
# The vLLM server has a different Torch/CUDA dependency contract from the
# SceneSmith/Blender process. Keep it in a versioned, isolated environment.
# CUDA 12.9 is supported by H100 and avoids the libcudart.so.13 dependency that
# is unavailable in the current ACP image.
ACP_VLLM_VERSION="0.22.1"
ACP_VLLM_TORCH_BACKEND="cu129"
ACP_VLLM_VENV_PATH="${SCENEEXPERT_VLLM_VENV_PATH:-$PROJECT_DIR/.venv-vllm-${ACP_VLLM_VERSION}-${ACP_VLLM_TORCH_BACKEND}}"
ACP_VLLM_AUTO_BOOTSTRAP=1
...
export SCENEEXPERT_VLLM_VERSION="$ACP_VLLM_VERSION"
export SCENEEXPERT_VLLM_TORCH_BACKEND="$ACP_VLLM_TORCH_BACKEND"
export SCENEEXPERT_VLLM_VENV_PATH="$ACP_VLLM_VENV_PATH"
export SCENEEXPERT_VLLM_AUTO_BOOTSTRAP=$ACP_VLLM_AUTO_BOOTSTRAP
export SCENEEXPERT_VLLM_EXECUTABLE=""
export SCENEEXPERT_VLLM_PYTHON=""
...
echo "vLLM runtime: version=${ACP_VLLM_VERSION}, backend=${ACP_VLLM_TORCH_BACKEND}"
echo "vLLM env: $ACP_VLLM_VENV_PATH"
