#!/usr/bin/env python3
"""
Standalone polygon geometry validation test.

Tests polygon_geometry.py directly without importing the full scenesmith stack.
"""
import json
import sys
from pathlib import Path

# Direct module import to avoid pydrake dependencies
sys.path.insert(0, str(Path(__file__).parent.parent / "scenesmith" / "floor_plan_agents" / "tools"))
...
def test_polygon_validation(
    ...
    config = polygon_geometry.PolygonValidationConfig(
    ...
    for case in irregular_cases:
        ...
        try:
            ...
            canonical = polygon_geometry.canonicalize_polygon(vertices, config)
            edges = polygon_geometry.polygon_edges(canonical)
        ...
        except polygon_geometry.PolygonValidationError as e:
            results["failed"] += 1
            failure = {
                "prompt_id": prompt_id,
