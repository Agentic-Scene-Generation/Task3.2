...
class _IndependentEditor:
    ...
    def generate_furniture_context_image(self, **kwargs):
        ...
        Image.new("RGB", (12, 12), color=(seed % 255, 30, 40)).save(output_path)
...
def test_prompts_use_relaxed_quality_rules_without_retry_feedback():
    ...
    assert "pixel-aligned invariant" in edit_prompt
...
def test_best_candidate_survives_lower_score_and_judge_error(tmp_path):
    ...
    assert output_path.read_bytes() == (
        tmp_path / "context_edited_attempt_01.png"
    ).read_bytes()
...
def test_normal_pass_stops_after_first_independent_candidate(tmp_path):
    module = pytest.importorskip(
        "scenesmith.furniture_agents.stateful_furniture_agent"
    )
    agent = _agent_for_quality_workflow(module)
    ...
    assert report["selection_mode"] == "passed"


def test_all_structurally_invalid_candidates_use_highest_score(tmp_path):
    module = pytest.importorskip(
        "scenesmith.furniture_agents.stateful_furniture_agent"
    )
    agent = _agent_for_quality_workflow(module)
    ...
    assert result == output_path
    assert output_path.read_bytes() == (
        tmp_path / "context_edited_attempt_02.png"
    ).read_bytes()
    report = json.loads((tmp_path / "context_image_quality.json").read_text())
    assert report["final_status"] == "best_candidate_fallback"
    assert report["accepted_attempt"] == 2
    assert report["best_attempt"] == 2
    assert report["best_score"] == 90
    assert report["selection_mode"] == "best_effort_fallback"
    assert report["designer_reference_path"] == str(output_path)
