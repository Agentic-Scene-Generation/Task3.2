...
_TRANSIENT_OPENAI_STATUS_CODES = frozenset({502, 503, 524})
_TRANSIENT_RETRY_DELAYS_ENV = "SCENEEXPERT_OPENAI_TRANSIENT_RETRY_DELAYS"


def _configured_transient_retry_delays() -> tuple[float, ...]:
    """Read opt-in retry delays for transient OpenAI-compatible API failures."""
    raw = os.environ.get(_TRANSIENT_RETRY_DELAYS_ENV, "").strip()
    if not raw:
        return ()
    ...
    try:
        ...
        console_logger.warning(
            "Ignoring invalid %s=%r; expected comma-separated non-negative seconds",
            _TRANSIENT_RETRY_DELAYS_ENV,
            raw,
        )
        return ()
    return delays
...
def _call_with_transient_retries(operation: Any) -> Any:
    ...
    for attempt in range(len(delays) + 1):
        try:
            ...
            if status_code not in _TRANSIENT_OPENAI_STATUS_CODES or attempt >= len(
                delays
            ):
                raise
            delay = delays[attempt]
            console_logger.warning(
                "OpenAI request returned HTTP %s; retrying in %g seconds (%d/%d)",
                status_code,
                delay,
                attempt + 1,
                len(delays),
...
async def _call_with_transient_retries_async(operation: Any) -> Any:
    delays = _configured_transient_retry_delays()
    for attempt in range(len(delays) + 1):
        try:
...
class _AsyncCompletionsWrapper:
    ...
    async def create(self, *args: Any, **kwargs: Any) -> Any:
        response = await _call_with_transient_retries_async(
            lambda: self._completions.create(*args, **kwargs)
        )
...
class _SyncResponsesWrapper:
    ...
    def create(self, *args: Any, **kwargs: Any) -> Any:
        response = _call_with_transient_retries(
            lambda: self._responses.create(*args, **kwargs)
        )
        _persist_response_sync(
            response,
            api_kind="responses",
