...
def score_category_ids(scores: CritiqueWithScores) -> list[str]:
    """Return schema-owned category IDs in the same order as ``get_scores``.

    ``CategoryScore.name`` is generated by the model and is therefore display
    text, not a storage key.  Deriving IDs from the concrete critique dataclass
    keeps persisted scores stable across Title Case, snake_case, translations,
    and otherwise semantically equivalent model output.
    """
    category_ids = [
        field.name
        for field in fields(scores)
        if field.name != "critique"
        and isinstance(getattr(scores, field.name, None), CategoryScore)
    ]
    if len(category_ids) != len(scores.get_scores()):
        raise ValueError(
            f"Score schema mismatch for {type(scores).__name__}: "
            f"{len(category_ids)} category fields but "
            f"{len(scores.get_scores())} returned scores"
        )
    return category_ids
