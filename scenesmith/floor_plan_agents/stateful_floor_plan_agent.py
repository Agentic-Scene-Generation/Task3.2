...
class StatefulFloorPlanAgent(BaseStatefulAgent, BaseFloorPlanAgent):
    ...
    async def generate_house_layout(self, prompt: str, output_dir: Path) -> HouseLayout:
        ...
        if reservation_validation.advisories:
            console_logger.warning(
                "Final floor-plan reservation advisories accepted: %s",
    ...
    async def _repair_reservation_validation(self, validation: Any) -> None:
        """Give the designer one bounded chance to repair hard reservations."""
        issue_details = json.dumps(validation.issues, sort_keys=True)
        issue_types = {
            str(issue.get("issue_type") or "") for issue in validation.issues
        }
        explicit_geometry = getattr(
            self.reservation_manifest, "explicit_geometry", None
        )
        if isinstance(self.reservation_manifest, dict):
            explicit_geometry = self.reservation_manifest.get("explicit_geometry")
        if isinstance(explicit_geometry, dict):
            geometry_is_immutable = bool(explicit_geometry.get("detected", False))
        else:
            geometry_is_immutable = bool(getattr(explicit_geometry, "detected", False))

        repair_steps = [
            "Use render_ascii() to identify exact wall labels and call validate() "
            "after every complete repair.",
        ]
        if "missing_exterior_entrance" in issue_types:
            repair_steps.append(
                "Use add_door() on a suitable exterior wall to restore an exterior entrance."
            )
        if "missing_explicit_windows" in issue_types:
            repair_steps.append(
                "Use add_window() to supply every explicitly required window."
            )
        if issue_types & {
            "implicit_window_budget_exceeded",
            "implicit_windows_per_wall_exceeded",
        }:
            repair_steps.append(
                "Use remove_window() for excess implicit windows; if needed, add one "
                "back on a different exterior wall with add_window()."
            )
        if issue_types & {
            "missing_matching_window_for_opening_adjacency",
            "insufficient_window_side_capacity",
        }:
            repair_steps.append(
                "Create or relocate the matching window with remove_window() and "
                "add_window() so one side retains the required continuous free span."
            )
        if "insufficient_opening_free_wall_capacity" in issue_types:
            repair_steps.append(
                "Move or remove conflicting doors/windows before consuming the reserved "
                "wall segment; keep the requested furniture anchor wall clear."
            )
        if "missing_opposed_opening_free_media_axis" in issue_types:
            repair_steps.append(
                "Relocate doors or windows to preserve opposed opening-free walls for "
                "the required media-viewing axis."
            )
        if "explicit_geometry_mismatch" in issue_types:
            repair_steps.append(
                "Restore the exact dimensions or polygon vertices specified by the user; "
                "do not scale or replace that footprint."
            )
        if (
            "insufficient_functional_zone_area" in issue_types
            and not geometry_is_immutable
        ):
            repair_steps.append(
                "Use resize_room() when a hard functional-zone area requirement is too "
                "small. If a required room type is missing, regenerate the complete room specification."
            )
        if geometry_is_immutable:
            repair_steps.append(
                "The prompt supplies immutable geometry. Functional-zone area advisories "
                "must not be repaired by resizing or replacing the footprint."
            )
        instruction = (
            ...
            "do not merely describe a fix. Preserve all requested rooms and "
            f"connections. {' '.join(repair_steps)} Continue until validate() reports "
            "future_capacity and opening_budget as ok.\n\n"
