...
def enforce_stage_brief_scope(
    ...
    """Apply deterministic stage ownership after model and memory guidance."""
    ...
    def generate_stage_brief(
        ...
        if self._structured_llm is not None:
            ...
            if result.value is None:
                console_logger.warning(
                    "Structured GlobalPlanner failed for %s; using fallback: %s: %s",
                    stage,
                    result.final_error_kind,
                    result.final_error,
                )
                return self._fallback_brief(context)
            ...
            return enforce_stage_brief_scope(reconciled, context.task_spec)
        ...
        try:
            ...
            return enforce_stage_brief_scope(brief, context.task_spec)
        except Exception as e:
            _append_llm_debug(
                build_llm_call_debug_record(
                    stage=stage,
