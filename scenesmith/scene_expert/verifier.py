...
class StageVerifier:
    ...
    def verify(
        ...
        blocking_findings = (
            critic_feedback.findings if self._visual_score_hard_gate else []
        )
        ...
        if (
            score_source not in {"vlm_critic", "deterministic_hard_check"}
            and (scores_path is not None or score_source != "unavailable")
        ):
            _add_issue_once(
                issues,
                VerifyIssue(
        ...
        hard_collision_evidence = str(provenance.get("hard_check_evidence", ""))
        if self._visual_score_hard_gate:
            hard_collision_evidence = "\n".join(
                part for part in (hard_collision_evidence, critique_summary) if part
            )
        if _critique_has_hard_collision(hard_collision_evidence):
            _add_issue_once(
                issues,
                VerifyIssue(
        ...
        if stage == "furniture":
            ...
            missing_evidence = str(provenance.get("hard_check_evidence", ""))
            if self._visual_score_hard_gate:
                missing_evidence = "\n".join(
                    part for part in (missing_evidence, critique_summary) if part
                )
            ...
            if (
                self._visual_score_hard_gate
                and prompt_following is not None
                and prompt_following < 8
            ):
                _add_issue_once(
                    issues,
                    VerifyIssue(
        ...
        visual_scores_pass = (
            visual_avg_score is not None
            and visual_avg_score >= self._pass_threshold
        ...
        console_logger.info(
            ...
            f"source={score_source} visual_gate={self._visual_score_hard_gate}",
            stage,
            (
