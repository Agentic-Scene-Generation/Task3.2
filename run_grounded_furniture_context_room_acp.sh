#!/usr/bin/env bash
# Run one full rectangular-room ACP acceptance case with grounded image layout.
#
# GPU 0: Qwen3.6-27B Q8, embedding, SceneSmith/Blender
# GPU 1: Qwen-Image-Edit and GroundingDINO
#
# Usage:
#   bash run_grounded_furniture_context_room_acp.sh
...
RUN_ID="${RUN_ID:-grounded_room_acp_$(date +%Y%m%d_%H%M%S)}"
STOP_GROUNDING_DINO_ON_EXIT="${STOP_GROUNDING_DINO_ON_EXIT:-true}"
DRY_RUN="${DRY_RUN:-false}"

# Code worktrees do not contain the large ignored binaries/models. By default,
# reuse them from the stable Task3.2 checkout while executing this branch's code.
...
if [ "$DRY_RUN" = "true" ]; then
    echo "[DRY RUN] room launcher validation passed"
...
echo "[OK] grounded room ACP run completed"
