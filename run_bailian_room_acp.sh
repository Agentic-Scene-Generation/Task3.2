#!/usr/bin/env bash
# Batch ACP test for Bailian furniture context images in rectangular room mode.
#
...
#   bash run_bailian_room_acp.sh
#
# Useful overrides:
#   MAX_CASES=5 bash run_bailian_room_acp.sh
#   PIPELINE_STOP_STAGE=furniture bash run_bailian_room_acp.sh
set -Eeuo pipefail
...
RUN_ID="${RUN_ID:-bailian_room_acp_$(date +%Y%m%d_%H%M%S)}"
CASES_DIR="${CASES_DIR:-$PROJECT_ROOT/tmp/acp_bailian_room/$RUN_ID}"
...
echo "========== ACP BAILIAN ROOM FURNITURE CONTEXT TEST =========="
echo "run id:                     $RUN_ID"
...
echo "[SCENES] starting room furniture batch with Bailian image generation"
...
echo "[OK] Bailian room ACP batch completed"
