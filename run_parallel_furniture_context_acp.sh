...
# Cloud backends do not need the local Qwen-Image-Edit service.
if [[ "${FURNITURE_CONTEXT_IMAGE_GENERATION_BACKEND:-}" =~ ^(bailian|openrouter)$ ]]; then
    echo "[CLOUD IMAGE] Using ${FURNITURE_CONTEXT_IMAGE_GENERATION_BACKEND} backend; skipping Qwen-Image-Edit service"
elif curl -fsS --max-time 2 "http://127.0.0.1:18020/ready" \
